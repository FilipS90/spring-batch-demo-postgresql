...
public class SpringBatchConfig {
    ...
    public FlatFileItemReader<PersonCsv> reader(
            @Value("#{jobParameters['targetFilePath']}") final FileSystemResource fileSystemResource) {
    ...
    private record DownloadCsvFileTasklet(URL sourceFileUrl, Path targetFilePath) implements Tasklet {
        @Override
        public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
            downloadCsvFile(sourceFileUrl, targetFilePath);
            return RepeatStatus.FINISHED;
        }

        private static void downloadCsvFile(final URL url, final Path targetFilePath) {
            try (InputStream in = url.openStream()) {
                Files.copy(in, targetFilePath, StandardCopyOption.REPLACE_EXISTING);
                logger.info("File {} has been downloaded successfully from {}", targetFilePath, url);
            } catch (IOException e) {
